...
export class AppRoleList implements OnInit {
  ...
  sortState: { field: string; order: number } = { field: 'permissionLevel', order: 1 };
  ...
  load(): void {
    ...
    this.service.query(this.filters).subscribe({
      ...
      error: () => {
        this.loading.set(false);
        this.messages.add({ severity: 'error', summary: '錯誤', detail: '載入角色清單失敗' });
      },
    });
  }
    ...
    this.confirm.confirm({
      ...
      accept: () => {
        this.service.delete(role.roleId).subscribe({
          next: () => {
            this.messages.add({ severity: 'success', summary: '成功', detail: '角色已刪除' });
            this.load();
          },
          error: () =>
